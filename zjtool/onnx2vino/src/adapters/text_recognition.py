...
class LPRAdapterZsy(Adapter):
    __provider__ = 'lpr_zsy'
    prediction_types = (CharacterRecognitionPrediction,)
    ...
    def process(self, raw, identifiers=None, frame_meta=None):
        ...
        result = []
        for identifier, output in zip(identifiers, predictions):
            decoded_out = self.decode(output)
            result.append(CharacterRecognitionPrediction(identifier, decoded_out))
        return result

    def decode(self, outputs):
        character = ['[blank]'] + list(
            "京沪津渝冀晋蒙辽吉黑苏浙皖闽赣鲁豫鄂湘粤桂琼川贵云藏陕甘青宁新0123456789ABCDEFGHJKLMNPQRSTUVWXYZ港学使警澳挂军北南广沈兰成济领民航空")
        preds_index = np.argmax(outputs, axis=1)
        char_list = []
        for i in range(outputs.shape[0]):
            if preds_index[i] != 0 and (not (i > 0 and preds_index[i - 1] == preds_index[i])):
                char_list.append(character[preds_index[i]])
